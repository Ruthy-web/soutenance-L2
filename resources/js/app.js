...
class AdvancedVirtualTour {
    ...
    setupUI() {
        ...
        this.minimapContainer.innerHTML =
            '<div class="minimap-title">Plan</div>';
    ...
    loadPanorama(panoramaKey, options = {}) {
        ...
        if (!panorama) {
            ...
            console.log(
                ...
                Object.keys(this.config.panoramas)
            );
            return;
        }
        ...
        if (
            panorama.image.startsWith("http://") ||
            panorama.image.startsWith("https://")
        ) {
        ...
        textureLoader.load(
            ...
            (texture) => {
                ...
                if (panorama.hotspots && Array.isArray(panorama.hotspots)) {
                    ...
                    console.log(
                        `✅ ${panorama.hotspots.length} hotspots créés`
                    );
                }
            ...
            (error) => {
                ...
                if (this.loaderElement) {
                    const loaderText =
                        this.loaderElement.querySelector(".loader-text");
                    if (loaderText) {
        ...
        this.hotspotGroup.children.forEach((child) => {
            if (child.userData.isRing) {
                ...
                child.material.opacity =
                    0.3 + Math.sin(child.userData.time * 2) * 0.15;
            } else {
                child.userData.time += 0.02;
    ...
    goToStart() {
        const startPanorama =
            this.config.startPanorama || Object.keys(this.config.panoramas)[0];
        if (startPanorama) {
    ...
    showInfoModal(content) {
        ...
        ${
            content.details
                ? `<div style="margin-top: 15px; padding: 15px; background: #f5f5f5; border-radius: 6px;">${content.details}</div>`
                : ""
        }
      `;
    ...
    showMediaModal(content) {
        ...
        if (modal && modalContent) {
          ...
          <img src="${imageSrc}" style="max-width: 100%; height: auto; border-radius: 8px;" alt="${
                    content.title || "Media"
                }">
          ${
              content.description
                  ? `<p style="margin-top: 15px; color: #666;">${content.description}</p>`
                  : ""
          }
        `;
            } else if (
...
document.addEventListener("DOMContentLoaded", () => {
    // Only initialize if viewer element exists (for virtual tour pages)
